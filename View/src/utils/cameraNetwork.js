...
const AUTO_GENERATED_GO2RTC_VIEW_REGEX = /^cam\d+$/i
...
const MJPEG_CAMERA_PATH_REGEX = /\.(mjpeg|mjpg)($|[?#])/i

export const isMjpegCameraUrl = (url) => {
  const value = (url || "").trim()
  return isHttpCameraUrl(value) && MJPEG_CAMERA_PATH_REGEX.test(value)
}


export const isAutoGeneratedGo2RtcViewUrl = (url) => {
  const value = (url || "").trim()
  ...
  try {
    ...
    return (
      ...
      /\/stream\.html$/i.test(parsedUrl.pathname) &&
      AUTO_GENERATED_GO2RTC_VIEW_REGEX.test(src)
    )
  } catch {
    return false
  }
...
export const resolveCameraManagementViewUrl = (camera) => {
  const urlView = String(camera?.urlView || "").trim()
  const streamUrl = String(camera?.streamUrl || "").trim()
  const hasDirectStreamUrl = isHttpCameraUrl(streamUrl) || streamUrl.startsWith("/")

  if (urlView && (!hasDirectStreamUrl || !isAutoGeneratedGo2RtcViewUrl(urlView))) {
    return urlView
  }
